...
const axxd = path.resolve(__dirname, '../');
const entry = path.join(axxd, 'components/style/index.less');
let content = fs.readFileSync(entry).toString();
const styles = glob.sync(path.join(axxd, 'components/*/style/index.less'));
content += '\n';
styles.forEach((style) => {
  content += `@import "${style}";\n`;
});
content += `@import "${path.join(axxd, 'site/theme/static/index.less')}";\n`;

less.render.call(less, content, {
  paths: [path.join(axxd, 'components/style')],
}).then(({ css }) => {
  return postcss([
    reducePlugin,
  ...
  const bezierEasing = fs.readFileSync(path.join(axxd, 'components/style/color/bezierEasing.less')).toString();
  const tinyColor = fs.readFileSync(path.join(axxd, 'components/style/color/tinyColor.less')).toString();
  const colorPalette = fs.readFileSync(path.join(axxd, 'components/style/color/colorPalette.less'))
    .toString()
